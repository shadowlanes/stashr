...
export function BookmarkCard({ bookmark, onArchive, onDelete }: BookmarkCardProps) {
  ...
  return (
    <Link href={`/article/${bookmark.id}`} className="block">
      <article className="group relative bg-white border border-gray-200 rounded-xl p-4 hover:shadow-md transition-shadow cursor-pointer">
        <div className="flex gap-4">
          ...
          <div className="flex-1 min-w-0">
            <h2 className="font-medium text-gray-900 group-hover:text-red-500 transition-colors line-clamp-2 leading-snug">
        ...
        <div className="absolute top-3 right-3 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <button
            onClick={(e) => { e.preventDefault(); onArchive(bookmark.id, !bookmark.isArchived); }}
